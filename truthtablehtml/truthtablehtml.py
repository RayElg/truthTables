...
from browser.html import TABLE, TR, TH, TD, P

def makeTable(event):
	table = TABLE()
	expression = (document["expression"].value).upper() #Get input
	L = getFinalList(getStrList(expression)) #Make matrix of T/F from input
	
    	
	#variable title rows
	table <= TR(TD(L[i][0]) for i in range(len(L)))	

    	#T/F rows
    	for row in range(len(L[0][1])): #There is as many rows as there is elements in each list in L
    	    table <= TR(TD(str(L[i][1][row])) for i in range(len(L)))

	document["zone"].clear()

	document["zone"] <= P(expression)
	document["zone"] <= table

document["parseButton"].bind("click",makeTable)
